...
CrewSwapFun csf = new CrewSwapFun();
	if("TPE".equals(uObj.getBase())){
	    csf.SwapRdListTPE(year, uObj.getEmpno());
	    ...
	    if("1".equals(rObjAL.getResultMsg())){
			...
			%>
			<li><a href="change_log_result.jsp?type=<%=obj.getFormtypeAL()%>&formno=<%=obj.getFormnoAL() %>" class="ui-btn ui-nodisc-icon ui-btn-icon-right ui-icon-list_btn_arrow_right_gray" data-ajax="false">
...
<%
	...
	//KHH
  		...
  		response.sendRedirect("KHHchange_log_list.jsp?myDate="+year);			
  	}
  	else{
